...
def create_pennant_chart(num, bullish=True):
    ...
    for i in range(15):
        if i < 7:
            ...
            decay = 1 - ((i - 7) / 5)
            body = np.random.uniform(-1.0, 1.0) * decay
...
def save_file(num, df, bullish):
    base_dir = '../results/short_pattern/pennant_pattern'
    label = 'bullish' if bullish else 'bearish'
    output_dir = os.path.join(base_dir, label)
    os.makedirs(output_dir, exist_ok=True)

    file_name = f'{label}_pennant_{(num+1):04d}.png'
    ...
    mpf.plot(
        ...
        type='candle',
        style='charles',
        title=f'{label.capitalize()} pennant',
        savefig=file_path
    )


def start():
    for num in tqdm(range(1000), desc='Generating Pennant Charts'):
        create_pennant_chart(num, bullish=True)
        create_pennant_chart(num, bullish=False)
    return ('Success!')
